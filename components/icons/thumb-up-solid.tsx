import React from 'react';

interface ThumbUpSolidProps {
    className: string;
}

const ThumbUpSolid: React.FC<ThumbUpSolidProps> = ({ className = '' }) => {
    return (
        <>
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" className={className}>
                <g fill="none">
                    <path opacity=".3" d="M13.34 4.66L9 9v10h9l3-7v-2h-8.77l1.11-5.34z" fill="currentColor"></path>
                    <path
                        d="M21 8h-6.31l.95-4.57.03-.32c0-.41-.17-.79-.44-1.06L14.17 1 7.59 7.59C7.22 7.95 7 8.45 7 9v10c0 1.1.9 2 2 2h9c.83 0 1.54-.5 1.84-1.22l3.02-7.05c.09-.23.14-.47.14-.73v-2c0-1.1-.9-2-2-2zm0 4l-3 7H9V9l4.34-4.34L12.23 10H21v2zM1 9h4v12H1V9z"
                        fill="currentColor"
                    ></path>
                </g>
            </svg>
        </>
    );
};

export default ThumbUpSolid;
